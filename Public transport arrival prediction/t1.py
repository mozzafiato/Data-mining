...
def weather():
    ...
    for i in range(df.shape[0]):
        ...
        if s.isspace():
            s = '0'
        dez_sneg[df['dan'][i]] = (float(d), float(s))
...
def generate(data):
    ...
    X = np.zeros(shape=(len(odhod), 56))
    p = prazniki()
    vreme = weather()

    for i in range(len(odhod)):
        ind = 2*odhod[i].hour
        if odhod[i].minute >= 30:
            ind += 1
        X[i][ind] = 1
        X[i][odhod[i].weekday() + 48] = 1
        X[i][55] = p[(odhod[i].day, odhod[i].month)]
        #datum = str(odhod[i].month) + '/' + str(odhod[i].day) + '/' + str(odhod[i].year)
        #if vreme[datum][0]>0:
        #    X[i][56] = 1
        #X[i][57] = vreme[datum][1]
